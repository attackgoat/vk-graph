...
float4 plasma(
    ...
    float color1 = (sin(dot(offset, float2(sin(time * 3.0), cos(time * 3.0))) * 0.02 + time * 3.0) + 1.0) / 2.0;
    float2 center = extent / 2.0 + float2(extent.x / 2.0 * sin(-time * 3.0) * radius, extent.y / 2.0 * cos(-time * 3.0) * radius);
