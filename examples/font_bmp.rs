...
fn main() -> anyhow::Result<()> {
    ...
    let smoke_pipeline = ComputePipeline::create(
        ...
        Shader::new_compute(
            glsl!(
                ...
                float smoothNoise(vec2 p) {
                    ...
                    return dot(
                        mat2(fract(sin(vec4(0, 1, 27, 28) + i.x + i.y * 27) * 1e5))
                            * vec2(1 - p.y, p.y),
                        vec2(1 - p.x, p.x)
                    );
                }
                ...
                float fractalNoise(vec2 p) {
                    return smoothNoise(p) * 0.57
                        + smoothNoise(p * 2.45) * 0.28
                        + smoothNoise(p * 6) * 0.15;
                }
        ...
        .specialization(SpecializationMap::new(subgroup_size.to_ne_bytes()).constant(0, 0, 4)),
    )?;
